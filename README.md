...
Полная README-документация по текущему состоянию demo-проекта **RoboPulse**. Документ составлен по фактическим файлам репозитория и не предполагает изменений бизнес-логики.

## 1. Назначение проекта

**RoboPulse** — демонстрационный стенд упрощенной MES-системы для диспетчирования производственных заказов, партий и поштучного выполнения операций по техпроцессу.
...
Проект является demo/MVP, а не готовой промышленной MES: часть production data хранится в JSON runtime storage, авторизация не реализована, часть legacy endpoints оставлена для совместимости.
...
```cmd
cd C:\Users\zamoc\Desktop\robolabs-mes-demo
...
Для первичной инициализации новой пустой БД текущий startup не выполняет `prisma db push` автоматически. Если стенд новый и схема еще не применена, используйте миграции или контролируемую demo-инициализацию, описанную ниже в разделах про Prisma.
...
- `backend/src/mes.service.ts` — основная бизнес-логика demo.
...
Production runs / units / unit operations для demo-сценария хранятся не в PostgreSQL, а в JSON runtime storage:
...
- Demo aliases в backend для сопоставления `RC800`, `209983`, `Multiholder`, `231265`, `Печь`, `FURNACE-DEMO`.
...
- Качество из `QualityRecord`, с demo fallback при отсутствии записей.
...
- Production runs/units синхронизируются в PostgreSQL-модели и legacy JSON/runtime-хранилище для совместимости demo-сценариев.
...
| `POSTGRES_PASSWORD` | postgres container | `robolabs_demo_password` | Пароль PostgreSQL; для стенда заменить |
| `DATABASE_URL` | backend / Prisma | `postgresql://robolabs:robolabs_demo_password@postgres:5432/robolabs_mes?schema=public` | Connection string Prisma |
...
| `AUTO_DB_PUSH` | backend entrypoint | `false` | Явное разрешение `npx prisma db push` при старте; только для контролируемой demo-инициализации |
...
Для локального demo можно оставить стандартные значения. Для LAN/серверного стенда обязательно заменить `POSTGRES_PASSWORD` и синхронно обновить `DATABASE_URL`.
...
$env:DATABASE_URL="postgresql://robolabs:robolabs_demo_password@localhost:5432/robolabs_mes?schema=public"
$env:PORT="3000"
$env:PRODUCTION_RUNS_FILE="C:\Users\zamoc\Desktop\robolabs-mes-demo\backend\src\data\production-runs.json"
npm install
...
```cmd
cd C:\Users\zamoc\Desktop\robolabs-mes-demo\backend
set DATABASE_URL=postgresql://robolabs:robolabs_demo_password@localhost:5432/robolabs_mes?schema=public
set PORT=3000
set PRODUCTION_RUNS_FILE=C:\Users\zamoc\Desktop\robolabs-mes-demo\backend\src\data\production-runs.json
npm install
npm run prisma:generate
npm run build
npm run start:dev
...
Для controlled demo initialization можно использовать `npx prisma db push`, но не как скрытый startup на стенде с данными.
...
```cmd
cd C:\Users\zamoc\Desktop\robolabs-mes-demo\frontend
...
`AUTO_DB_PUSH=true` использовать только для контролируемой demo-инициализации новой/тестовой БД. Не включать как обычный режим production-like стенда.
...
Destructive reset допустим только для одноразового локального demo, когда данные точно не нужны, после backup и с явным force flag.
...
Sample Excel содержит demo-заказ:
...
## 14. Безопасность и ограничения demo-версии
...
- Пароль PostgreSQL из `.env.example` демонстрационный и должен быть заменен на стенде.
...
- Production runs/units уже представлены SQL-моделями, но до завершения SQL-first переключения часть demo-совместимости может читать/писать legacy JSON.
...
Этот раздел предназначен для ИИ-агента, который будет переносить текущий проект `RobolabsMesDemo` на сервер, доступный по SSH alias `ttm-mini`, разворачивать стенд и настраивать его для дальнейшей эксплуатации.
...
- корень проекта: `robolabs-mes-demo/`;
...
Если старого Docker volume нет или стенд переносится как чистый demo, backup-шаг зафиксировать как `SKIPPED: clean deployment`.
...
```bash
sudo mkdir -p /opt/robolabs-mes-demo
sudo chown -R "$USER":"$USER" /opt/robolabs-mes-demo
```
...
rsync -avz --delete \
  ...
  --exclude "*.log" \
  ./ ttm-mini:/opt/robolabs-mes-demo/
```

Если `rsync` недоступен на Windows, использовать архив:

```powershell
tar --exclude=frontend/node_modules --exclude=backend/node_modules --exclude=frontend/dist --exclude=backend/dist --exclude=frontend/test-results -czf robolabs-mes-demo.tar.gz .
scp .\robolabs-mes-demo.tar.gz ttm-mini:/tmp/robolabs-mes-demo.tar.gz
ssh ttm-mini "mkdir -p /opt/robolabs-mes-demo && tar -xzf /tmp/robolabs-mes-demo.tar.gz -C /opt/robolabs-mes-demo"
```

После передачи на сервере проверить:

```bash
cd /opt/robolabs-mes-demo
ls -la
...
Если это чистый demo-стенд и нужны стартовые справочники, выполнить seed осознанно:
...
```bash
scp robolabs_mes.dump ttm-mini:/opt/robolabs-mes-demo/robolabs_mes.dump
```
...
```bash
scp production-runs.backup.json ttm-mini:/opt/robolabs-mes-demo/production-runs.backup.json
ssh ttm-mini "cd /opt/robolabs-mes-demo && docker compose stop backend && docker run --rm -v robolabs_mes_runtime_data:/data -v /opt/robolabs-mes-demo:/backup alpine sh -c 'mkdir -p /data && cp /backup/production-runs.backup.json /data/production-runs.json'"
```
...
Если HTTPS использует self-signed certificate, браузер может показывать предупреждение. Для demo-стенда это допустимо, если пользователь явно принимает сертификат. Для production-нужд настроить нормальный сертификат и reverse proxy.
...
- проект лежит в `/opt/robolabs-mes-demo`;
