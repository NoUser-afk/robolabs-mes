...
Импорт заказов выполняется через вкладку `Импорт Excel` и endpoint `POST /api/import/orders-excel`.
...
Импорт техпроцессов выполняется из раздела `Номенклатура` через кнопку `Загрузить техпроцесс из Excel`.

Endpoints:

- `POST /api/import/techprocess-excel/preview` — проверка файла, запись истории preview, без создания версии.
- `POST /api/import/techprocess-excel` — сохранение новой версии техпроцесса в режиме `draft` или `active`.

Формат шаблона: `data/samples/techprocess-template.xlsx`.

Workbook содержит:

- лист `Process`: `equipment`, `productCode`, `processId`, `category`, `versionComment`, `notes`, `activate`;
- лист `Operations`: `sequence`, `operationId`, `name`, `section`, `level`, `partOrAssembly`, `normHours`, `previousOperationCodes`, `nextOperationCodes`, `groupCapable`, `x`, `y`.

Импорт техпроцессов:

- сохраняет данные в `NomenclatureProcessVersion`;
- не изменяет уже созданные `ProductionRun`;
- фиксирует каждую проверку/загрузку в `TechProcessImportBatch`;
- блокирует импорт при дублирующихся операциях, циклах или ссылках на отсутствующие операции.
...
| `ImportBatch` | История импортов заказов из Excel |
| `TechProcessImportBatch` | История проверок и загрузок техпроцессов из Excel |
