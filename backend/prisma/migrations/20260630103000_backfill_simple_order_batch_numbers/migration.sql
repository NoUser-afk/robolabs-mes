WITH ranked AS (
  SELECT
    ...
    ROW_NUMBER() OVER (
      PARTITION BY "orderNumber", "productCode", COALESCE("orderYear", EXTRACT(YEAR FROM "createdAt")::INTEGER)
...
SET
  ...
  "orderScopeKey" = CASE
    WHEN run."orderId" IS NOT NULL THEN 'id:' || run."orderId" || ':product:' || ranked."productCode"
    ELSE 'number:' || ranked."orderNumber" || ':' || ranked.batch_year || ':product:' || ranked."productCode"
  END,
...
WHERE run.id = ranked.id
  AND (run."orderBatchNo" IS NULL OR run."orderBatchCode" IS NULL OR run."batchNumber" ~ '^RUN-');
...
SET
  "orderYear" = ranked.batch_year,
  "orderScopeKey" = CASE
    WHEN ranked."orderId" IS NOT NULL THEN 'id:' || ranked."orderId" || ':product:' || ranked."productCode"
    ELSE 'number:' || ranked."orderNumber" || ':' || ranked.batch_year || ':product:' || ranked."productCode"
  END,
  "orderBatchNo" = ranked.batch_no,
  "orderBatchCode" = ranked."orderNumber" || '-' || ranked.batch_no
...
WHERE record.id = ranked.id
  AND (record."orderBatchNo" IS NULL OR record."orderBatchCode" IS NULL);
